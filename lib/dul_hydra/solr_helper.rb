module DulHydra::SolrHelper
  ...
  def af_model_filter(solr_params, user_params)
    solr_params[:fq] ||= []
    solr_params[:fq] << "+active_fedora_model_ssim:#{user_params[:model]}"
  end

  def preservation_events_filter(solr_params, user_params)
    solr_params[:fq] ||= []
    solr_params[:fq] << "+is_preservation_event_for_ssim:\"info:fedora/#{user_params[:object_id]}\""
  end

  def targets_filter(solr_params, user_params)
    solr_params[:fq] ||= []
    solr_params[:fq] << "+is_external_target_for_ssim:\"info:fedora/#{user_params[:object_id]}\""
  ...
  def children_filter(solr_params, user_params)
    ...
    solr_params[:fq] << "is_member_of_ssim:#{object_uri} OR is_member_of_collection_ssim:#{object_uri} OR is_part_of_ssim:#{object_uri}"
