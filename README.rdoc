...
To add to and/or rearrange items on the object and datastream context navigation menus (while retaining the basic behavior),
follow these steps.

* Create an initializer for fcrepo_admin at config/initializers/fcrepo_admin.rb. You should start with this content:
  ...
  The settings that control the items on the context menus are:

    FcrepoAdmin.object_context_nav_items = [:summary, :datastreams, :permissions, :associations, :audit_trail]
    FcrepoAdmin.datastream_context_nav_items = [:current_version, :summary, :content, :download, :edit, :upload, :history]
...
* To add a custom navigation item, start by adding your own symbol to the approriate array setting.  Then override the 
  appropriate helper method (see section above on overriding fcrepo_admin helpers) -- for the object context nav, override 
  #custom_object_context_nav_item in ObjectsHelper; for the datastream context nav, override #custom_object_content_nav_item
